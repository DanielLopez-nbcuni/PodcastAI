## Podcast AI built on NotebookLLama

- Step 1: Script Prep: Use `Llama-3.2-1B-Instruct` to pre-process a PDF and save it in a `.txt` file.
...
NOTE: we prompt the 1B model to not modify the text or summarize it, strictly clean up extra characters or garbage characters that might get picked due to encoding from PDF. Please see the prompt in Notebook 1 for more details.
...
NOTE: You can also use `Llama-3.1-8B-Instruct` model, recommend experimenting and trying if you see any differences. The 70B model was used here because it gave slightly more creative podcast transcripts for the tested examples.

- Step 3: Style Re-Writer: Use `Llama-3.1-8B-Instruct` model to make the transcript more dramatic
...
NOTE: These models were chosen based on a sample prompt and worked best, newer models might sound better.
...
- Better prompting
- Support for ingesting a website, audio file, YouTube links and more.
